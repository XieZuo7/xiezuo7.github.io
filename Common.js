...
function Fen1(fen1id) {
    $(".nei").addClass("hidden");
    if (fen1id > 0) {



        var datastr = "fen1id=" + escape(fen1id);
        var data = AjaxWeb("Fen1", datastr);
        $("#XiangLei").html(data);
    } else {


        $("#XiangLei").html("<option value='0'>请选择项目类别</option>");
    }
}

function Fen2(fen2id) {
    if (fen2id > 0) {

        var datastr2 = "fen2id=" + escape(fen2id);
        var data2 = AjaxWeb("SaiXing", datastr2);

        $("#SaiXing").val(data2);
        if (data2 == "0") {
            $(".geren").removeClass("hidden");
            $(".tuan").addClass("hidden");
        } else {
            $(".geren").addClass("hidden");
            $(".tuan").removeClass("hidden");
        }

        $(".nei").removeClass("hidden");


        var datastr = "fen2id=" + escape(fen2id);
        var data = AjaxWeb("Fen2", datastr);
        $("#ZuBie").html(data);
        ...
        var datastr2 = "fen2id=" + escape(fen2id);
        var data2 = AjaxWeb("YaoMa2", datastr2);
        if (data2 == "1") {
            $("#IsXuYao").val("1");
            $(".yaoqingma").removeClass("hidden");
        } else {
            $("#IsXuYao").val("0");
            $("#YaoMa").val("");
            $(".yaoqingma").addClass("hidden");
        }
    } else {
        $("#IsXuYao").val("0");
        $(".yaoqingma").addClass("hidden");
        $("#YaoMa").val("");
        $(".nei").addClass("hidden");
        $("#ZuBie").html("<option value='0'>请选择组别</option>");
    }
}
